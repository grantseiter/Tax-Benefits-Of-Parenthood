...
#### File Author:
*Grant M. Seiter · American Enterprise Institute* (September 2020 (Modified February 2021))

#### Description: 
For households earning less than $125,000 annually, Biden proposed a refundable
          tax credit of up to half a family's child care costs. It would max out at $8,000 for
          one child or $16,000 for two or more children. Families earning between $125,000 and
          $400,000 will qualify for partial credits.
          # Limited by PUF e32800 cap at $6000
...
### records.py
```python
self.e32800_alt *= gfv['ATXPY']    # ADD MODIFIED VAR e32800_alt for CDCC_new 

```

### data.py
```python
# ADD MODIFIED VAR e32800_alt for CDCC_new: 
        np.random.seed(87)         
        self.e32800_alt = np.where(self.e32800 == 3000,  
            np.random.triangular(1,1.172731,2.036388,size=248591) * self.e32800,  
            np.where(self.e32800 == 6000, np.random.triangular(1,1.172731,2.036388,size=248591) * self.e32800, self.e32800)) 
```
...
def IITAX(c59660, c11070, c10960, personal_refundable_credit, ctc_new,
          cdcc_new, rptc, brctc, nyctc, romneyca,
          c09200, payrolltax,
    ...
    refund = (eitc + c11070 + c10960 +
              personal_refundable_credit + ctc_new + cdcc_new + rptc + brctc + nyctc + romneyca)
    iitax = c09200 - refund
...
from taxcalc.calcfunctions import (TaxInc, SchXYZTax, GainsTax, AGIsurtax,
                                   ...
                                   RomneyChildAllowance, PersonalTaxCredit, SchR,
